...
import { APIPromise } from 'abundai/core/api-promise';

import util from 'node:util';
import Abundai from 'abundai';
import { APIUserAbortError } from 'abundai';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['ABUNDAI_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Abundai) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new Abundai({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['ABUNDAI_LOG'] = 'debug';
      const client = new Abundai({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['ABUNDAI_LOG'] = 'not a log level';
      const client = new Abundai({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'ABUNDAI_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Abundai({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['ABUNDAI_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Abundai({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['ABUNDAI_BASE_URL'] = 'https://example.com/from_env';
      const client = new Abundai({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['ABUNDAI_BASE_URL'] = ''; // empty
      const client = new Abundai({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.abund.ai');
    });

    test('blank env variable', () => {
      process.env['ABUNDAI_BASE_URL'] = '  '; // blank
      const client = new Abundai({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.abund.ai');
    });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () => new Abundai({ apiKey: 'My API Key', environment: 'production' }),
      ).toThrowErrorMatchingInlineSnapshot(
        `"Ambiguous URL; The \`baseURL\` option (or ABUNDAI_BASE_URL env var) and the \`environment\` option are given. If you want to use the environment you must pass baseURL: null"`,
      );
    ...
    test('in request options overridden by client options', () => {
      const client = new Abundai({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['ABUNDAI_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Abundai({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Abundai({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Abundai({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['ABUNDAI_API_KEY'] = 'My API Key';
    const client = new Abundai();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['ABUNDAI_API_KEY'] = 'another My API Key';
    const client = new Abundai({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Abundai({ apiKey: 'My API Key', fetch: testFetch });
