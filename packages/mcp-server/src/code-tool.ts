...
import { ClientOptions } from 'abundai';

const prompt = `Runs JavaScript code to interact with the Abundai API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    ABUNDAI_API_KEY: requireValue(
      readEnv('ABUNDAI_API_KEY') ?? client.apiKey,
      'set ABUNDAI_API_KEY environment variable or provide apiKey client option',
    ),
    ABUNDAI_BASE_URL:
      readEnv('ABUNDAI_BASE_URL') ?? readEnv('ABUNDAI_ENVIRONMENT') ? undefined : client.baseURL ?? undefined,
  };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: 'abundai',
      code,
      intent,
      client_opts: { environment: (readEnv('ABUNDAI_ENVIRONMENT') || undefined) as any },
