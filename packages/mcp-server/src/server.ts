...
export { ClientOptions } from 'abundai';
...
async function getInstructions() {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/abundai',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the abundai MCP server. You will use Code Mode to help the user perform
...
export async function initMcpServer(params: {
  ...
  let client = new Abundai({
    ...{ environment: (readEnv('ABUNDAI_ENVIRONMENT') || undefined) as any },
    logger,
