...
```sh
export ABUNDAI_API_KEY="My API Key"
export ABUNDAI_ENVIRONMENT="production"
npx -y abundai-mcp@latest
```
...
```json
{
  "mcpServers": {
    "abundai_api": {
      ...
      "args": ["-y", "abundai-mcp"],
      "env": {
        "ABUNDAI_API_KEY": "My API Key",
        "ABUNDAI_ENVIRONMENT": "production"
      }
    }
  }
}
...
[Open VS Code](https://vscode.stainless.com/mcp/%7B%22name%22%3A%22abundai-mcp%22%2C%22command%22%3A%22npx%22%2C%22args%22%3A%5B%22-y%22%2C%22abundai-mcp%22%5D%2C%22env%22%3A%7B%22ABUNDAI_API_KEY%22%3A%22My%20API%20Key%22%7D%7D)
...
```
claude mcp add abundai_mcp_api --env ABUNDAI_API_KEY="My API Key" -- npx -y abundai-mcp
```
...
| ------------------- | ------------------------ | --------------- |
| `x-abundai-api-key` | `apiKey` | BearerAuth |
